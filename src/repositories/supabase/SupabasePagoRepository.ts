...
export class SupabasePagoRepository implements PagoRepository {
  ...
  /** El trigger trg_actualizar_saldo_fiado descuenta el saldo del fiado. */
  async create(nuevo: NuevoPago): Promise<Pago> {
    const { data, error } = await this.supabase
      ...
      .select()
      .single();
    lanzarSiError(error, { entidad: 'Pago' });
    return data as Pago;
  }
  ...
  async getByCliente(clienteId: number): Promise<Pago[]> {
    const { data, error } = await this.supabase
      ...
      .order('fecha_pago', { ascending: false });
    lanzarSiError(error, { entidad: 'Pago' });
    return (data as (Pago & { fiados?: unknown })[]).map(({ fiados: _fiados, ...pago }) => pago);
  }
}
